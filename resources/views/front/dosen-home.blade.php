...
@section('pagetitle', @isset($pageTitle) ? $pagetitle : 'Dosen | Dashboard')
@push('stylesheets')
    <!-- CSS Libraries -->
        ...
        <div class="pb-20">
            <div class="table-responsive">
                <table class="table table-striped" aria-label="Data Pengajuan">
                    ...
                    <tbody>
                        @forelse ($sempros as $item)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $item->mahasiswa->nim }}</td>
                                <td>{{ $item->mahasiswa->nama }}</td>
                                <td>{{ $item->mahasiswa->prodi->jenjang }} {{ $item->mahasiswa->prodi->nama }}</td>
                        ...
                        @empty
                            <div class="alert alert-danger">
                                Data Pendaftar Skripsi belum tersedia.
                            </div>
                        @endforelse
                    </tbody>
                </table>
